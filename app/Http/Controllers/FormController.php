...
class FormController extends Controller
{
    ...
    protected function BtnAction($id){
        $btn = DB::select("SELECT DISTINCT t1.id, t2.prev_state, isnull(t2.prev_state) as isPrevstate, t2.current_state, isnull(t2.current_state) as isCurrentstate, t3.to_state, isnull(t3.to_state) as isTostate, t1.ord_vehicle, t3.action_name FROM `flows` t1 INNER JOIN (SELECT * FROM states WHERE id IN (SELECT max(id) from states GROUP BY ord_vehicle)) t2 on t2.ord_vehicle = t1.ord_vehicle INNER JOIN tb_state_action t3 on t3.from_state = t2.current_State WHERE t1.ord_vehicle = ?", [$id]);
        return $btn;
    }
}
